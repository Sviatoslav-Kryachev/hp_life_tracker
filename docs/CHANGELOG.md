...
## [2024] - Интерактивный календарь и улучшения UI

### Добавлено
- **Интерактивный календарь с фильтрами периодов**
  - Переключатель между периодами: Неделя / Месяц / Год
  - Визуальные отметки активных дней (зелёные точки/оттенки)
  - Интенсивность цвета зависит от количества заработанного XP
  - Выделение текущего дня/месяца
  - Клик по дню/месяцу для просмотра деталей

### Добавлено
- **Детальная информация по дням**
  - Модальное окно с полной статистикой дня
  - Общая статистика: заработано, потрачено, время, итого
  - Список всех сессий активности с деталями (название, время, длительность, XP)
  - Список всех покупок наград (название, время, потраченный XP)
  - API endpoint `/xp/day/{date}` для получения детальной информации

### Добавлено
- **API endpoints для календаря**
  - `GET /xp/month` - статистика за текущий месяц (по дням)
  - `GET /xp/year` - статистика за текущий год (по месяцам)
  - `GET /xp/day/{date}` - детальная информация по конкретному дню

### Изменено
- **Улучшен дизайн блока "Серия дней"**
  - Структурированное отображение: заголовок, текущая серия (по центру), рекорд, сообщение
  - Текущая серия отображается крупным шрифтом с подписью "дней"
  - Рекорд отделён разделительной линией

### Изменено
- **Блок "Рекомендации"**
  - Зелёный дизайн (градиентный фон, зелёная граница)
  - Зелёные кнопки запуска активности с градиентом
  - Улучшенная функция запуска активности из рекомендаций
  - Визуальная обратная связь (уведомления, прокрутка, подсветка)

### Изменено
- **Выравнивание заголовков блоков**
  - Все заголовки виджетов выровнены по центру:
    - "Сегодня" (солнышко и текст)
    - "Неделя" / "Календарь"
    - "Прогресс"
    - "Серия дней"
    - "Рекомендации"
  - Единообразный дизайн всех блоков

### Изменено
- **Блок "Мои цели"**
  - Кнопка добавления цели перемещена рядом с заголовком
  - Сортировка целей по дате достижения (ближайшие сверху)
  - Выполненные цели отображаются внизу списка

### Изменено
- **Навигация и скролл**
  - Клик по "Админ-панель" в футере прокручивает к админ-панели
  - Плавная прокрутка к нужным разделам

### Технические детали
- Файлы: `app/routers/xp.py` (новые endpoints), `static/app.js` (функции `loadCalendar()`, `changeCalendarPeriod()`, `showDayDetails()`), `static/index.html` (обновлённый календарь)
